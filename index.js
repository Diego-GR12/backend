...
async function generarRespuestaIA(
  ...
  if (modeloReq && nombreModelo !== modeloReq) {
    console.warn(
      `[Gen IA] Modelo '${modeloReq}' no está en la lista de permitidos o es inválido, usando por defecto: ${MODELO_POR_DEFECTO}. Modelos permitidos: ${MODELOS_PERMITIDOS.join(
        ", "
      )}`
    );
  }
  ...
  const fechaActualFormateada = ahora.toLocaleDateString(
    localeParaFormato,
    opcionesFecha
  );
  const horaActualFormateada = ahora.toLocaleTimeString(
    localeParaFormato,
    opcionesHora
  );

  const infoFechaHoraContexto =
    lang === "en"
  ...
  console.log(
    `[Gen IA] ➡️ Enviando ${contenidoGemini.length} partes a Gemini (${nombreModelo}). Contexto fecha/hora para IA: "${infoFechaHoraContexto}"`
  );
...
async function generarImagenClipdrop(promptTexto) {
  ...
  try {
    ...
    const extension = tipoMime.includes("png")
      ? "png"
      : tipoMime.includes("jpeg") || tipoMime.includes("jpg")
      ? "jpg"
      : "out";
