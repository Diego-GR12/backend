...
const {
  PORT: PUERTO = 3001,
  DB_HOST, // Ya no se usan si usas Supabase directamente
  DB_USER, // Ya no se usan
  DB_PASSWORD, // Ya no se usan
  DB_NAME, // Ya no se usan
...
const MAX_CARACTERES_POR_PDF = 10000; // Considerar ajustar según uso real
const MAX_LONGITUD_CONTEXTO = 30000; // Límite para contexto enviado a Gemini
...
if (!API_KEY)
  console.warn(
    "⚠️ [Startup] ADVERTENCIA: API_KEY (Google GenAI) no configurada."
  );
if (!SUPABASE_URL)
...
if (!CLIPDROP_API_KEY)
  console.warn(
    "⚠️ [Startup] ADVERTENCIA: CLIPDROP_API_KEY (para imágenes) no configurada."
  );
...
try {
  if (API_KEY) {
    ...
    clienteIA = null;
    console.warn("⚠️ GoogleGenerativeAI NO inicializado (falta API_KEY).");
  }
} catch (error) {
  console.error(
    "🚨 FATAL: Error inicializando GoogleGenerativeAI:",
    error.message
  );
  clienteIA = null;
}
...
try {
  if (SUPABASE_URL && SUPABASE_KEY) {
    ...
    supabase = null;
    console.warn(
      "⚠️ Supabase NO inicializado (faltan SUPABASE_URL o SUPABASE_KEY)."
    );
  }
} catch (error) {
  console.error("🚨 FATAL: Error inicializando Supabase:", error.message);
  supabase = null;
}
...
); // Simplificado para devolver origen o true
...
const autenticarToken = (req, res, next) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, usuarioToken) => {
    ...
    req.usuario = usuarioToken; // Añade info del usuario al request
    next();
  });
};
...
}).array("archivosPdf"); // Espera un campo llamado 'archivosPdf' con potencialmente múltiples archivos

// Middleware Multer genérico (sin filtro específico de tipo aquí)
...
[directorioSubidas, directorioImagenesGeneradas].forEach((dir) => {
  if (!existsSync(dir)) {
    try {
      ...
      console.error(`🚨 FATAL: No se pudo crear directorio ${dir}:`, error);
    }
  } else {
...
async function extraerTextoDePDF(rutaArchivo) {
  ...
  try {
    await fs.access(rutaArchivo); // Check existence
    ...
    if (error.code === "ENOENT") {
      console.error(
        `❌ [PDF Extract] Archivo NO ENCONTRADO: ${rutaArchivo}. Verificar persistencia.`
      );
      return {
        texto: null,
...
// Generar Contexto Combinado de PDFs
async function generarContextoPDF(idUsuario, nombresArchivosUnicos) {
  if (
    !idUsuario ||
    !nombresArchivosUnicos ||
    nombresArchivosUnicos.length === 0
  )
    return "";
  ...
  try {
    ...
    if (dbError) {
      throw new Error(`Error BD obteniendo archivos: ${dbError.message}`);
    }
    if (!archivosDB || archivosDB.length === 0) {
      console.warn("[Context PDF] No se encontraron archivos en DB.");
      return "[Archivos especificados no encontrados]";
    }
    ...
    for (const nombreUnico of nombresArchivosUnicos) {
      ...
      if (!nombreOriginal) {
        console.warn(`[Context PDF] Falta info original para ${nombreUnico}`);
        continue;
      }
      ...
      try {
        ...
        console.error(
          `[Context PDF] Excepción procesando ${nombreUnico}:`,
          extractErr
        );
      }
    }
...
// Generar Respuesta de Texto con IA (Google Gemini)
async function generarRespuestaIA(
  ...
  const langStrings =
    idioma === "en"
      ? {
          ...
          label: "Question:",
          error: "...",
        } // Copiar strings completos de versiones anteriores
      : {
          systemBase:
  ...
  if (textoPDF) {
    ...
    if (textoPDF.length > MAX_LONGITUD_CONTEXTO)
      console.warn("[Gen IA] Contexto PDF truncado.");
  ...
  console.log(
    `[Gen IA] Enviando ${contenidoGemini.length} partes a Gemini (${nombreModelo})`
  );
  try {
    ...
    console.warn(
      `[Gen IA] Respuesta vacía/bloqueada. Block: ${blockReason}, Finish: ${finishReason}`
    );
    throw new Error(
      langStrings.error +
        (blockReason
    ...
    ); // Relanzar error para handler global
  }
}

// Generar y Guardar Imagen usando Clipdrop
async function generarImagenClipdrop(promptTexto) {
  if (!CLIPDROP_API_KEY)
    throw new Error(
      "Servicio de generación de imágenes (Clipdrop) no disponible."
    );
  ...
  try {
    ...
    // Podrías añadir 'style_preset': 'photographic', 'anime', 'comic-book', etc.
    ...
    if (!response.ok) {
      ...
      // Simplificar mensaje para el usuario final
      if (status === 401 || status === 403)
        throw new Error("Error de autenticación con el servicio de imágenes.");
      if (status === 402)
        throw new Error(
          "Límite de uso gratuito del servicio de imágenes alcanzado."
        );
      if (status === 429)
        throw new Error(
          "Servicio de imágenes sobrecargado. Intente más tarde."
        );
      throw new Error("Error al contactar el servicio de imágenes.");
    }
    ...
    const nombreArchivo = `${Date.now()}-clipdrop-${promptTexto
      .substring(0, 15)
    ...
    return {
      ...
      url: `/generated_images/${nombreArchivo}`,
    };
  } catch (error) {
    console.error("[Img Gen Clipdrop] Catch Error:", error.message);
    // Asegurarse de que el error que se propaga sea útil para el usuario final
...
app.post("/api/register", async (req, res, next) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("usuarios")
      .insert([{ nombre_usuario: username, contrasena_hash: hash }])
      .select("id")
      .single();
    if (error) {
      if (error.code === "23505")
...
app.post(
  "/api/files",
  autenticarToken,
  upload.array("archivosPdf"),
  async (req, res, next) => {
    // Usar upload genérico
    if (!supabase) return res.status(503).json({ error: "BD no disponible." });
    try {
      ...
      // Podríamos añadir validación de tipo aquí si es necesario para esta ruta específica
      const records = files.map((f) => ({
        usuario_id: uid,
        nombre_archivo_unico: f.filename,
      ...
      if (error) {
        ...
        throw error; // Re-lanzar para handler global
      }
      res
        .status(200)
        .json({
          mensaje: `Se ${files.length > 1 ? "guardaron" : "guardó"} ${
            files.length
          } archivo(s).`,
        });
    } catch (error) {
...
app.delete(
  "/api/files/:nombreArchivoUnico",
  autenticarToken,
  async (req, res, next) => {
    if (!supabase) return res.status(503).json({ error: "BD no disponible." });
    const { id: idUsuario } = req.usuario;
    const { nombreArchivoUnico } = req.params;
    if (!nombreArchivoUnico)
      return res.status(400).json({ error: "Falta nombre de archivo." });
    try {
      ...
      if (findError || !archivo)
        return res
          .status(404)
          .json({ error: "Archivo no encontrado o no autorizado." });
      ...
      try {
        ...
        if (fsErr.code !== "ENOENT")
          console.error("Error borrando físico:", fsErr);
      } // Best effort
...
app.delete(
  ...
  async (req, res, next) => {
    ...
    if (!idConv)
      return res.status(400).json({ error: "Falta ID conversación." });
    try {
      ...
      res.json({
        message: `Conversación ${
          count > 0 ? "eliminada" : "no encontrada/autorizada"
        }.`,
        deleted: count > 0,
      });
    } catch (error) {
...
app.put(
  ...
  async (req, res, next) => {
    ...
    if (!id || !nuevoTitulo?.trim())
      return res.status(400).json({ error: "Datos inválidos." });
    const tituloLimpio = nuevoTitulo.trim().substring(0, 100); // Truncar si excede
    try {
      const { error, count } = await supabase
        .from("conversaciones")
        .update({
...
app.post(
  ...
  subirPdf,
  async (req, res, next) => {
    if (!supabase || !clienteIA)
      return res.status(503).json({ error: "Servicio BD o IA no disponible." });
    ...
    try {
      if (archivosSeleccionados)
        archivosSeleccionadosArray = JSON.parse(archivosSeleccionados || "[]");
      if (!Array.isArray(archivosSeleccionadosArray)) throw new Error();
    } catch (e) {
      return res
        .status(400)
    ...
    try {
      ...
      if (!conversationId) {
        ...
        const { data: convData, error: convErr } = await supabase
          ...
          .single();
        if (convErr)
          throw new Error(`Error creando conversación: ${convErr.message}`);
        conversationId = convData.id;
        isNewConversation = true;
      }

      // 2. Guardar prompt usuario si existe
      if (prompt) {
        await supabase
          .from("mensajes")
          .insert([
            { conversacion_id: conversationId, rol: "user", texto: prompt },
          ])
          .catch((e) => console.error("Error guardando msg user:", e));
      }

      // 3. Procesar PDFs nuevos
      const archivosNuevos = req.files || [];
      if (archivosNuevos.length > 0) {
        const records = archivosNuevos.map((f) => ({
        ...
        const { error: fErr } = await supabase
          ...
          .insert(records);
        if (fErr) {
          archivosNuevos.forEach(async (f) => {
            try {
              await fs.unlink(f.path);
            } catch (e) {}
          });
          throw new Error("Error guardando info PDF.");
        }
      }

      // 4. Generar contexto PDF
      const archivosCtx = [
        ...archivosSeleccionadosArray,
        ...archivosNuevos.map((f) => f.filename),
      ].filter(Boolean);
      let contextoPDF = "";
      if (archivosCtx.length > 0) {
        contextoPDF = await generarContextoPDF(usuarioId, archivosCtx);
      }

      // 5. Validar si hay algo para procesar
      if (!prompt && !contextoPDF) {
        return res
          .status(400)
          .json({ error: "Se requiere prompt o archivos PDF." });
      }
      ...
      const { data: hist, error: histErr } = await supabase
        ...
        .order("fecha_envio", { ascending: true });
      if (histErr)
        throw new Error(`Error cargando historial: ${histErr.message}`);

      // 7. Generar respuesta IA
      const promptReal =
        prompt ||
        (idioma === "es" ? "Resume los archivos." : "Summarize files.");
      ...
      // 8. Guardar respuesta modelo
      await supabase
        .from("mensajes")
        .insert([
          { conversacion_id: conversationId, rol: "model", texto: respIA },
        ])
        .catch((e) => console.error("Error guardando msg model:", e));

      // 9. Enviar respuesta
      res
        .status(200)
        .json({ respuesta: respIA, isNewConversation, conversationId });
    } catch (error) {
      next(error);
    } // Dejar al handler global
  }
);

// Generar Imagen (usando Clipdrop)
app.post("/api/generateImage", autenticarToken, async (req, res, next) => {
  ...
  if (!CLIPDROP_API_KEY)
    return res
      .status(503)
      .json({ error: "Servicio de imágenes no configurado." });
...
app.use(
  "/generated_images",
  express.static(directorioImagenesGeneradas, { maxAge: "1h" })
); // Cache de 1 hora
...
app.use((err, req, res, next) => {
  ...
  // Mejorar mensajes y códigos
  if (err instanceof multer.MulterError) {
    if (err.code === "LIMIT_FILE_SIZE") {
      statusCode = 413;
      mensajeUsuario = `Archivo grande (Máx: ${TAMANO_MAX_ARCHIVO_MB} MB).`;
    } else if (err.message === "Solo se permiten archivos PDF.") {
      statusCode = 400; /* mensaje ya es bueno */
    } else {
      statusCode = 400;
      mensajeUsuario = `Error subida: ${err.field || ""} ${err.code}`;
    }
  } else if (
    ...
    statusCode = 401; // O 403? 401 es más común para keys inválidas.
    ...
    statusCode = 400; // Asumir Bad Request si contiene "inválido"
  } else if (statusCode === 500 && mensajeUsuario.includes("fetch")) {
    // Ocultar detalles de fetch
    mensajeUsuario = "Error de red contactando servicio externo.";
  }
  // Añadir lógica para errores específicos de BD si es necesario (ej. unique constraint)
...
app.listen(PORT, () => {
  console.log(
    `\n🚀 Servidor corriendo en puerto ${PORT} | ${
      isDev ? "MODO DESARROLLO" : "MODO PRODUCCIÓN"
    }`
  );
  ...
  console.log(
    ` Clipdrop Imagen: ${
      CLIPDROP_API_KEY
        ? "✅ OK (Key presente)"
        : "❌ NO OK (Verificar CLIPDROP_API_KEY)"
    }`
  );
  console.log(`----------------------\n`);
});
