...
console.log(
  ...
  JWT_SECRET
    ? `${JWT_SECRET.substring(0, 3)}... (long: ${JWT_SECRET.length})`
    : "NO CARGADO!"
);
if (!JWT_SECRET || JWT_SECRET.length < 32)
  console.warn("⚠️ JWT_SECRET no definido o inseguro!");
...
if (!CLIPDROP_API_KEY)
  console.warn("⚠️ CLIPDROP_API_KEY (para imágenes) no configurada.");
...
try {
  if (API_KEY) {
    ...
    console.log("✅ GoogleGenerativeAI creado.");
  } else {
    clienteIA = null;
    console.warn("⚠️ GoogleGenerativeAI NO inicializado (sin API_KEY).");
  }
} catch (e) {
  console.error("🚨 Error GoogleGenerativeAI:", e.message);
  clienteIA = null;
}
...
try {
  ...
  console.log(
    "[Env Vars Check Before Supabase Init] SUPABASE_URL:",
    SUPABASE_URL
      ? `Cargada (longitud: ${SUPABASE_URL.length})`
      : "NO CARGADA O VACÍA"
  );
  console.log(
    "[Env Vars Check Before Supabase Init] SUPABASE_KEY:",
    SUPABASE_KEY
      ? `Cargada (primeros 3 chars: ${SUPABASE_KEY.substring(
          0,
          3
        )}..., longitud: ${SUPABASE_KEY.length})`
      : "NO CARGADA O VACÍA"
  );
  ...
  if (SUPABASE_URL && SUPABASE_KEY) {
    ...
    supabase = null;
    console.warn(
      "⚠️ Supabase NO inicializado (sin URL/KEY). Esto causará errores en operaciones de DB y Storage."
    );
  }
} catch (e) {
...
const autenticarToken = (req, res, next) => {
  ...
  if (!JWT_SECRET) {
    console.error("[Auth] JWT_SECRET falta!");
    return res.status(500).json({ error: "Error auth server." });
  }
...
async function generarContextoPDF(idUsuario, rutasSupabaseArchivos) {
  ...
  if (!supabase) {
    console.warn("[Context PDF] Supabase no disponible.");
  ...
  try {
    const { data: archivosDB, error: dbError } = await supabase
      ...
      .in("nombre_archivo_unico", rutasSupabaseArchivos);
    if (dbError) {
      console.error("[Context PDF] Supabase error (meta):", dbError.message);
      return "[Error al recuperar metadatos PDF]";
    }
    ...
    for (const rutaSupabase of rutasSupabaseArchivos) {
      ...
      if (downloadError) {
        console.warn(
          `[Context PDF] Supabase download error ${rutaSupabase}:`,
      ...
      try {
        ...
        console.warn(
          `[Context PDF] Parse error ${rutaSupabase}:`,
          parseError.message
        );
      }
    }
    ...
    console.error("[Context PDF] Exception:", err);
    return "[Error al generar contexto PDF]";
  }
}
...
async function generarRespuestaIA(
  ...
  if (modeloReq && nombreModelo !== modeloReq)
    console.warn(
      `[Gen IA] Modelo no válido ('${modeloReq}'), usando: ${MODELO_POR_DEFECTO}`
    );
  ...
  if (textoPDF && textoPDF.length > MAX_LONGITUD_CONTEXTO)
  ...
  try {
    ...
    const errorDetail = blockReason
      ? `Bloqueo: ${blockReason}`
      : finishReason
      ? `Finalización: ${finishReason}`
      : "Respuesta inválida";
    ...
    throw new Error(
      `${langStrings.error}. (Detalle: ${error.message || "Desconocido"})`
    );
  }
}
...
async function generarImagenClipdrop(promptTexto) {
  if (!CLIPDROP_API_KEY)
    throw new Error(
      "Servicio de imágenes (Clipdrop) no disponible (sin API key)."
    );
  ...
  try {
    ...
    const extension = tipoMime.includes("png")
      ? "png"
      : tipoMime.includes("jpeg")
      ? "jpeg"
      : "out";
    const nombreArchivoImagenOriginal = `${Date.now()}-clipdrop-${promptTexto
      .substring(0, 15)
      .replace(/[^a-z0-9]/gi, "_")}.${extension}`;
    const supabaseImagePath = nombreArchivoImagenOriginal;

    console.log(
      `[Supabase Storage Img Upload Debug] Intentando subir '${nombreArchivoImagenOriginal}' como '${supabaseImagePath}' al bucket '${SUPABASE_IMAGES_BUCKET}'`
    );
    const { error: uploadError } = await supabase.storage
      .from(SUPABASE_IMAGES_BUCKET)
      .upload(supabaseImagePath, bufferImagen, {
        contentType: tipoMime,
        upsert: true,
      });
    if (uploadError) {
      console.error(
        `[Supabase Storage Img Upload Fail] Error detallado al subir '${supabaseImagePath}':`,
        JSON.stringify(uploadError, null, 2)
      );
      throw new Error(
        `Error al guardar la imagen generada en el almacenamiento: ${uploadError.message}`
      );
    }
    console.log(
      `[Supabase Storage Img Upload Success] Subida '${supabaseImagePath}' exitosamente.`
    );

    const { data: publicUrlData } = supabase.storage
      .from(SUPABASE_IMAGES_BUCKET)
      .getPublicUrl(supabaseImagePath);
    if (!publicUrlData || !publicUrlData.publicUrl) {
      console.error(
        `[Supabase Storage] Error obteniendo URL pública para ${supabaseImagePath}. Datos devueltos:`,
        publicUrlData
      );
      // Si no podemos obtener la URL, el archivo subido es inútil y podría causar problemas. Intentar borrarlo.
      await supabase.storage
        .from(SUPABASE_IMAGES_BUCKET)
        .remove([supabaseImagePath])
        .catch((remErr) =>
          console.error(
            `Error al intentar borrar imagen ${supabaseImagePath} tras fallo de getPublicUrl:`,
            remErr
          )
        );
      throw new Error(
        "Error al obtener la URL de la imagen generada (publicUrlData es nulo o no tiene publicUrl)."
      );
    }

    console.log(
      `[Img Gen Clipdrop Axios] Guardada en Supabase. URL Pública: ${publicUrlData.publicUrl}`
    );
    return {
      fileName: nombreArchivoImagenOriginal,
      url: publicUrlData.publicUrl,
    };
  } catch (error) {
    let status = 500;
    let errorMsgParaUsuario = "Error desconocido generando imagen.";
    if (
      error.message.includes("almacenamiento") ||
      error.message.includes("URL de la imagen")
    ) {
      errorMsgParaUsuario = error.message;
      ...
      if (responseData) {
        if (Buffer.isBuffer(responseData)) {
          try {
            ...
            clipdropError = responseData.toString("utf-8");
          }
        } else if (typeof responseData === "object" && responseData.error) {
          clipdropError = responseData.error;
        } else if (typeof responseData === "string") {
          clipdropError = responseData;
        }
      }
      console.error(
        `[Img Gen Clipdrop Axios] Error API Clipdrop (${status}):`,
        clipdropError
      );
      if (status === 400)
        errorMsgParaUsuario = "Prompt inválido para Clipdrop.";
      else if (status === 401 || status === 403)
        errorMsgParaUsuario = "API Key de Clipdrop inválida.";
      else if (status === 402)
        errorMsgParaUsuario = "Límite Clipdrop alcanzado.";
      else if (status === 429)
        errorMsgParaUsuario = "Límite de tasa Clipdrop. Intente más tarde.";
      else
        errorMsgParaUsuario = `Error servicio imágenes: ${clipdropError.substring(
          0,
          150
        )}`;
    } else if (error.request) {
      console.error(
        "[Img Gen Clipdrop Axios] Sin respuesta de Clipdrop:",
        error.message
      );
      errorMsgParaUsuario = "No se pudo contactar el servicio de imágenes.";
    } else {
      console.error("[Img Gen Clipdrop Axios] Error interno:", error.message);
      errorMsgParaUsuario =
        error.message || "Error interno en solicitud de imagen.";
    }
...
app.post("/api/register", async (req, res, next) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("usuarios")
      .insert([
        { nombre_usuario: username, contrasena_hash: contrasenaHasheada },
      ])
      .select("id")
      .single();
    if (error) {
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      for (const f of archivosRecibidos) {
        ...
        const nombreSupa = `${usuarioId}/${Date.now()}-${f.originalname
          .normalize("NFD")
          .replace(/[\u0300-\u036f]/g, "")
          .replace(/[^a-z0-9.\-_]/gi, "_")}`;
        console.log(
          `[Storage Upload Debug /api/files] Intentando subir '${f.originalname}' como '${nombreSupa}' al bucket '${SUPABASE_PDF_BUCKET}'`
        );
        if (!f.buffer) {
          console.error(
            `[Storage Upload Debug /api/files] Error: f.buffer no existe para el archivo ${f.originalname}.`
          );
          errStor.push({
            originalName: f.originalname,
            generatedName: nombreSupa,
            errorDetails: { message: "f.buffer is missing" },
          });
          continue;
        }
        const { error: uE } = await supabase.storage
          .from(SUPABASE_PDF_BUCKET)
          .upload(nombreSupa, f.buffer, {
            contentType: f.mimetype,
            upsert: false,
          });
        if (uE) {
          console.error(
            `[Storage Upload Fail /api/files] Error detallado al subir '${nombreSupa}' (Original: ${f.originalname}):`,
          ...
          console.log(
            `[Storage Upload Success /api/files] Subido '${nombreSupa}' exitosamente.`
          );
          resultadosSubidaDB.push({
            usuario_id: usuarioId,
            nombre_archivo_unico: nombreSupa,
            nombre_archivo_original: f.originalname,
          });
        }
      }
      if (resultadosSubidaDB.length > 0) {
        const { error: dbInsertError } = await supabase
          .from("archivos_usuario")
          .insert(resultadosSubidaDB);
        if (dbInsertError) {
          console.error(
            "[DB Insert PDF Meta /api/files] Error:",
            dbInsertError
          );
          for (const subido of resultadosSubidaDB) {
            const { error: removeError } = await supabase.storage
              .from(SUPABASE_PDF_BUCKET)
              .remove([subido.nombre_archivo_unico]);
            if (removeError)
              console.error(
                "Error limpiando PDF de Storage tras fallo DB (/api/files):",
                removeError.message
              );
          }
          return next(
            new Error(
              `Error guardando metadatos en DB: ${dbInsertError.message}`
            )
          );
        }
      }
      if (errStor.length > 0) {
        return res.status(resultadosSubidaDB.length > 0 ? 207 : 400).json({
          mensaje:
            resultadosSubidaDB.length > 0
              ? "Algunos PDF subidos, otros fallaron."
              : "No se pudo subir ningún PDF.",
          subidos: resultadosSubidaDB.map((r) => r.nombre_archivo_original),
          errores: errStor.map((e) => ({
            originalName: e.originalName,
            error: e.errorDetails?.message || "Error desconocido en la subida",
          })),
        });
      }
...
app.delete(
  "/api/files/:rutaSupabaseArchivo(.*)",
  autenticarToken,
  async (req, res, next) => {
    // (.*) para capturar rutas con /
    if (!supabase) return res.status(503).json({ error: "BD no disponible" });
    const idUsuario = req.usuario.id;
    const rutaSupabaseArchivo = req.params.rutaSupabaseArchivo;
    if (!rutaSupabaseArchivo)
      return res
        .status(400)
        .json({ error: "Ruta de archivo Supabase no especificada." });
    try {
      const { data: archivoMeta, error: metaError } = await supabase
        .from("archivos_usuario")
        .select("id")
        .eq("usuario_id", idUsuario)
        .eq("nombre_archivo_unico", rutaSupabaseArchivo)
        .single();
      if (metaError || !archivoMeta) {
        if (metaError && metaError.code !== "PGRST116") {
          console.error("[Delete File Meta Error]", metaError);
          throw metaError;
        }
        return res
          .status(404)
          .json({ error: "Archivo no encontrado o no pertenece al usuario." });
      }
      const { error: storageDeleteError } = await supabase.storage
        .from(SUPABASE_PDF_BUCKET)
        .remove([rutaSupabaseArchivo]);
      // Incluso si storageDeleteError existe (ej: archivo ya no estaba en storage), intentamos borrar de DB
      if (storageDeleteError) {
        console.warn(
          "[Supabase Storage Delete Warning/Error]",
      ...
      const { error: dbDeleteError } = await supabase
        .from("archivos_usuario")
        .delete()
        .eq("id", archivoMeta.id);
      if (dbDeleteError) {
        console.error("[DB Delete PDF Meta Error]", dbDeleteError.message);
        throw new Error(
          `Error eliminando metadato PDF de DB: ${dbDeleteError.message}.`
        );
      }
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (!convOwner)
        return res
          .status(404)
          .json({ error: "Conversación no encontrada o no autorizada." });
...
app.post(
  ...
  async (req, res, next) => {
    if (!supabase) {
      console.error(
        "Error: Cliente Supabase no inicializado en /api/generateText"
      );
      return res
        .status(503)
    ...
    if (!clienteIA) {
      console.error(
        "Error: Cliente GoogleGenerativeAI no inicializado en /api/generateText"
      );
    ...
    const usuarioId = req.usuario.id;
    console.log(
      "[Storage Upload Debug /api/generateText] Iniciando. UsuarioId:",
      usuarioId
    );
    ...
    if (archivosSeleccionados) {
      try {
        archivosSelParseados =
          typeof archivosSeleccionados === "string"
        ...
        if (typeof archivosSeleccionados === "string")
          return res
            .status(400)
            .json({ error: "Formato archivosSeleccionados inválido." });
        archivosSelParseados = [];
      }
    }
    ...
    try {
      if (!conversationId) {
        const { data, error } = await supabase
          ...
          .insert([
            {
              usuario_id: usuarioId,
              titulo: prompt?.trim().substring(0, 50) || "Conversación",
            },
          ])
          .select("id")
          .single();
      ...
      if (prompt?.trim()) {
        ...
        if (userMsgErr)
          console.error("Error guardando msg usr:", userMsgErr.message);
      }
      if (archivosPdfNuevosSubidos.length > 0) {
        for (const f of archivosPdfNuevosSubidos) {
          console.log(
            "[Storage Upload Debug /api/generateText] Dentro del bucle de archivos. Archivo actual:",
            f.originalname
          );
          const nombreSupa = `${usuarioId}/${Date.now()}-${f.originalname
            .normalize("NFD")
            ...
            .replace(/[^a-z0-9.\-_]/gi, "_")}`;
          console.log(
            `[Storage Upload Debug /api/generateText] Intentando subir '${f.originalname}' como '${nombreSupa}' al bucket '${SUPABASE_PDF_BUCKET}'`
          );
          if (!f.buffer) {
            console.error(
              `[Storage Upload Debug /api/generateText] Error: f.buffer no existe para ${f.originalname}.`
            );
            errStor.push({
              originalName: f.originalname,
              generatedName: nombreSupa,
          ...
          const { error: uE } = await supabase.storage
            .from(SUPABASE_PDF_BUCKET)
            .upload(nombreSupa, f.buffer, { contentType: f.mimetype });
          if (uE) {
            console.error(
              `[Storage Upload Fail /api/generateText] Error detallado al subir '${nombreSupa}' (Original: ${f.originalname}):`,
              JSON.stringify(uE, null, 2)
            );
            errStor.push({
              originalName: f.originalname,
            ...
            console.log(
              `[Storage Upload Success /api/generateText] Subido '${nombreSupa}'.`
            );
        ...
        if (regDB.length > 0) {
          ...
          if (iE) {
            ...
            for (const r of rutasSupabaseNuevosArchivos) {
              ...
              if (remErr)
                console.error(
                  "Fallo limpieza Storage tras error DB (/api/generateText):",
                  remErr.message
                );
            }
        ...
        if (errStor.length > 0) {
          console.warn(
            `Fallaron en Storage durante generateText: ${errStor
              .map((e) => e.originalName)
              .join(", ")}`
          );
        }
      }
      const todasRutasSupaCtx = [
        ...archivosSelParseados,
        ...rutasSupabaseNuevosArchivos,
      ].filter(Boolean);
      const contextoPDF = await generarContextoPDF(
        usuarioId,
        todasRutasSupaCtx
      );
      if (!prompt?.trim() && (!contextoPDF || contextoPDF.startsWith("[Error")))
        return res
          .status(400)
          .json({ error: "Prompt o PDF válidos requeridos." });
      const { data: hist, error: errH } = await supabase
        ...
        .eq("es_error", false)
        .order("fecha_envio", { ascending: true });
      if (errH) throw new Error("Error cargando historial: " + errH.message);
      const promptIA =
        prompt || (idioma === "es" ? "Resume archivos." : "Summarize files.");
      ...
      if (modelMsgErr)
        console.error("Error guardando msg model:", modelMsgErr.message);
      if (errStor.length > 0) {
        return res
          .status(207)
          .json({
            ...
            isNewConversation,
            conversationId,
            uploadErrors: errStor.map((e) => ({
              originalName: e.originalName,
              error:
                e.errorDetails?.message || "Error desconocido en la subida",
            })),
          });
      }
      res
        .status(200)
        .json({ respuesta: respuestaIA, isNewConversation, conversationId });
    } catch (error) {
      next(error);
    }
  }
...
app.post("/api/generateImage", autenticarToken, async (req, res, next) => {
  if (!supabase || !CLIPDROP_API_KEY)
    return res.status(503).json({ error: "Servicio(s) no disponible(s)." });
  const { prompt, conversationId: inputConvId } = req.body;
  if (!prompt?.trim())
    return res.status(400).json({ error: "Prompt inválido." });
  if (!inputConvId)
    return res.status(400).json({ error: "ID de conversación requerido." });
  const conversationId = parseInt(inputConvId);
  if (isNaN(conversationId))
    return res.status(400).json({ error: "ID de conversación inválido." });
  try {
    const { data: cO, error: oE } = await supabase
      .from("conversaciones")
      .select("id")
      .eq("id", conversationId)
      .eq("usuario_id", req.usuario.id)
    ...
    if (!cO)
      return res
        .status(404)
        .json({ error: "Conversación no encontrada/autorizada." });
    const resultadoImagen = await generarImagenClipdrop(prompt.trim());
    const { data: msgG, error: msgIE } = await supabase
      .from("mensajes")
      .insert([
        {
          conversacion_id: conversationId,
          rol: "model",
          texto: resultadoImagen.url,
          tipo_mensaje: "image",
        },
      ])
      .select("id")
      .single();
    if (msgIE) {
      console.error("[GenerateImage] Error DB:", msgIE.message);
      return res
        .status(207)
        .json({
          message: "Imagen generada pero error guardándola en conv.",
          fileName: resultadoImagen.fileName,
          imageUrl: resultadoImagen.url,
          errorDB: msgIE.message,
        });
    }
    res.json({
...
app.use((err, req, res, next) => {
  ...
  if (err instanceof multer.MulterError) {
    if (err.code === "LIMIT_FILE_SIZE") {
      ...
      msgU =
        errL === "en"
          ? `File large (Max: ${TAMANO_MAX_ARCHIVO_MB}MB).`
          : `Archivo grande (Máx: ${TAMANO_MAX_ARCHIVO_MB}MB).`;
    } else if (
    ...
    err.message.includes("no configurado")
  )
    scode = 503;
    ...
    err.message.includes("pago") ||
    err.message.includes("créditos")
  ) {
    scode = 402;
    msgU = "Límite de uso gratuito.";
  } else if (
    err.message.includes("Demasiadas solicitudes") ||
    err.message.includes("sobrecargado") ||
    ...
    (err.code.startsWith("2") || err.code.startsWith("PGR"))
  ) {
    console.warn(
      "Error DB (Supabase/Postgres):",
      err.code,
      err.detail || err.hint
    );
    msgU = err.message.includes("constraint")
      ? "Conflicto de datos."
      : "Error en BD.";
    if (err.code === "23505") scode = 409;
    else scode = 500;
  }
...
app.listen(PORT, () => {
  ...
  console.log(
    ` Supabase: ${
      supabase
        ? `✅ OK (PDFs en '${SUPABASE_PDF_BUCKET}', Imágenes en '${SUPABASE_IMAGES_BUCKET}')`
        : "❌ NO OK (Verificar URL/KEY)"
    }`
  );
  console.log(
    ` Google GenAI: ${clienteIA ? "✅ OK" : "❌ NO OK (Verificar API_KEY)"}`
  );
  console.log(
    ` Clipdrop Imagen: ${
      CLIPDROP_API_KEY ? "✅ OK" : "❌ NO OK (Verificar CLIPDROP_API_KEY)"
    }`
  );
  console.log(`----------------------\n`);
});
